        ...
        <div className="overflow-hidden rounded-[28px] bg-text-main px-6 py-10 shadow-xl sm:px-10 sm:py-14 md:px-14">
          ...
          <div className="flex flex-col items-stretch gap-10 lg:flex-row lg:gap-14">
            ...
            {/* Hero image: below text on mobile, right column on desktop
                where it stretches to match the text column's height so no
                empty dark space is left below a shorter image. Uses
                explicit width/height (not `fill`): an absolutely-positioned
                `fill` image with height:100% does not reliably resolve
                against a flex/grid item sized only via `stretch` in this
                Chromium build (confirmed via computed style — height stayed
                at the image's intrinsic aspect ratio instead of 100%); an
                in-flow <Image> with h-full does.

                Single poster on every breakpoint (mobile/tablet used to get
                a separate landscape photo via a <picture>/<source media>
                pair — that's what still showed the old two-employee photo
                on phones after desktop moved to this poster). The poster is
                portrait (1086×1448, 3:4) — the wrapper's aspect ratio
                matches that on mobile/tablet (object-contain, so it always
                renders whole: no cropped hotline/address/service list),
                and switches to lg:aspect-auto lg:flex-1 on desktop where
                flex stretch takes over instead. */}
            <div className="relative mx-auto aspect-[3/4] w-full max-w-lg overflow-hidden rounded-2xl shadow-lg lg:mx-0 lg:aspect-auto lg:max-w-none lg:flex-1">
              <Image
