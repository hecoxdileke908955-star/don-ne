  ...
  return <header className="sticky top-0 z-40 w-full border-b border-gray-100 bg-white/95 backdrop-blur-md">
    <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
      ...
      <nav className="hidden items-center gap-6 text-sm font-medium text-text-main md:flex">
        <Link href="/dich-vu" className="transition hover:text-primary">Dịch Vụ</Link><Link href="/bang-gia" className="transition hover:text-primary">Bảng Giá 2026</Link><Link href="/khu-vuc" className="transition hover:text-primary">Khu Vực Phục Vụ</Link><Link href="/cam-nang" className="transition hover:text-primary">Cẩm Nang</Link><Link href="/gioi-thieu" className="transition hover:text-primary">Giới Thiệu</Link><Link href="/lien-he" className="transition hover:text-primary">Liên Hệ</Link>
      </nav>
    ...
    {mobileMenuOpen && <div className="space-y-3 border-t border-gray-100 bg-white px-4 py-4 text-sm md:hidden">
      {[['/dich-vu', 'Dịch Vụ'], ['/bang-gia', 'Bảng Giá 2026'], ['/khu-vuc', 'Khu Vực Phục Vụ'], ['/cam-nang', 'Cẩm Nang Vệ Sinh'], ['/gioi-thieu', 'Giới Thiệu'], ['/lien-he', 'Liên Hệ']].map(([href, label]) => <Link key={href} href={href} onClick={() => setMobileMenuOpen(false)} className="block py-1.5 font-medium text-text-main">{label}</Link>)}
